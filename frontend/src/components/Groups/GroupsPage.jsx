...
export default function GroupsPage() {
  ...
  const handleSaveGroupName = async (e) => {
    ...
    setProcessing(true);
    try {
      await api.updateGroup(selectedGroup.id, { group_name: editGroupName });
      setShowEditGroup(false);
      // Refresh data
      const updated = { ...selectedGroup, group_name: editGroupName };
    ...
    <>
      ...
      {showEditGroup && (
        <div className="modal-overlay" onClick={() => setShowEditGroup(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2 className="modal-title">✏️ แก้ไขชื่อกลุ่ม</h2>
            <form onSubmit={handleSaveGroupName}>
              <div className="form-group" style={{ marginBottom: 16 }}>
                <label className="form-label">ชื่อกลุ่ม</label>
                <input className="form-input" value={editGroupName} onChange={e => setEditGroupName(e.target.value)} required autoFocus />
              </div>
              <div className="modal-actions">
